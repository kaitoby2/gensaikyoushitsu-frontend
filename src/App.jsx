...
export default function App() {
  ...
  /** ============ 画面遷移（ログイン/登録/メイン/管理者） ============ */
  const [screen, setScreen] = useState("login"); // "login" | "create" | "main" | "admin"
  ...
  const onPickPhoto = async (file) => {
    ...
    try {
      ...
      fd.append("image", file); // FastAPI 側は "image" で受け取り
  ...
  const scoreNow = async () => {
    ...
    try {
      ...
      await saveResponse({
        ...
        score: d,
        advice: [],
        group_id: (groupId || "").trim() || null, // ★ 保存に group_id を含める
      });
    } catch (error) {
  ...
  const [adminUsers, setAdminUsers] = useState([]); // /admin/users
                    ...
                    <button
                      onClick={async () => {
                        ...
                        const rows = await apiGetAuth(
                          `/admin/responses?user_id=${encodeURIComponent(u.user_id)}`,
                          adminToken
                        );
        ...
        {adminScreen === "detail" && (
          <div>
            <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ margin: 0 }}>
                ユーザー詳細：{adminSelectedUser?.user_name || "(未設定)"} (<code>{adminSelectedUser?.user_id}</code>)
              </h2>
  ...
  return (
    <div className="app-shell patterned-bg" style={baseTextStyle}>
        ...
        <div className="header-inner">
          <div className="brand">
            <span className="brand-title">
              <span className="first">減</span>
              <span className="second">災</span>教室
            </span>
            <small>『わかる』から『できる』へ</small>
          </div>
          <div className="langs">
            <button type="button" className="pill jp">
              Japanese
            </button>
            <button type="button" className="pill en">
              English
            </button>
          </div>
        </div>
      ...
      <div className="container">
        <p className={health === "Backend OK" ? "ok" : "ng"}>Backend: {health}（{API_BASE}）</p>
            ...
            (isScenarioFinished ? (
              <div style={{ marginTop: 12 }}>
                ...
                <div className="row" style={{ gap: 8, marginTop: 12 }}>
                  <button type="button" onClick={() => setIsUnityOpen(true)} className="btn-primary" style={{ padding: "10px 14px", fontSize: 18 }}>
                    3Dシミュレーションを開始
                  </button>
                  <button
                    type="button"
                ...
                {activeScenario?.source_note && <p className="muted" style={{ fontSize: 14, marginTop: 12 }}>{activeScenario.source_note}</p>}
              </div>
            ) : (
              <div>
                {activeScenario?.title && <div style={{ fontWeight: 700, marginBottom: 6 }}>{activeScenario.title}</div>}
                {activeScenario?.summary && <p className="muted" style={{ marginTop: 0 }}>{activeScenario.summary}</p>}
                      ...
                      {isLast ? (
                        <div className="row" style={{ gap: 8, flexWrap: "wrap", marginTop: 4, justifyContent: "center" }}>
                          {(node.choices ?? []).map((c, i) => (
                            <button key={`${node.id}-${i}`} onClick={() => jumpTo(c.next, c.label)} className="btn-primary" style={{ padding: "10px 14px" }}>
                              {c.label}
                            </button>
                          ))}
                        </div>
                      ) : (
                        <div className="muted" style={{ fontSize: 13 }}>(経過)</div>
                      )}
                    </div>
                  );
                })}
                {activeScenario?.source_note && <p className="muted" style={{ fontSize: 14 }}>{activeScenario.source_note}</p>}
              </div>
            ))}
        </section>
        ...
        <section className="card">
          ...
          <form onSubmit={analyzeInventory} className="grid" style={{ gap: 16 }}>
            <NumberField label="人数" value={inv.persons} min={1} step={1} onChange={(val) => setInv((v) => ({ ...v, persons: val }))} />
            <NumberField label="500ml 本数" value={inv.bottles500} min={0} step={1} onChange={(val) => setInv((v) => ({ ...v, bottles500: val, overrideLiters: false }))} />
            <NumberField label="2L 本数" value={inv.bottles2l} min={0} step={1} onChange={(val) => setInv((v) => ({ ...v, bottles2l: val, overrideLiters: false }))} />
            <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input type="checkbox" checked={inv.overrideLiters} onChange={(e) => setInv((v) => ({ ...v, overrideLiters: e.target.checked }))} />
              直接 L 数を入力する
            </label>
            {inv.overrideLiters && <NumberField label="水 (L)" value={inv.water_l} min={0} step={0.1} decimals={1} onChange={(val) => setInv((v) => ({ ...v, water_l: val }))} />}
        ...
        <section className="card">
            ...
            return (
              <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ marginTop: 6, display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ padding: "4px 10px", borderRadius: 999, background: info.color, color: "#fff", fontWeight: 700 }}>
                    {info.label}
                  </span>
          ...
          {!scored && <div className="muted" style={{ marginTop: 8 }}>※ 友達や家族との比較は、スコア計算後に表示されます。</div>}
        ...
        {showComparison && (
          <section className="card">
            ...
            <div className="callout" style={{ border: "1px solid #d0ebff", background: "#f0f9ff", borderRadius: 12, padding: "12px 14px", marginBottom: 12 }}>
            ...
            <div className="subcard" style={{ border: "1px solid #eee", borderRadius: 10, padding: 12, marginBottom: 10 }}>
              ...
              <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
                <input placeholder="チーム名（例：家族グループ）" value={groupName} onChange={(e) => setGroupName(e.target.value)} style={{ padding: 6, minWidth: 240 }} disabled={!setupDone} />
                <button onClick={createGroup} disabled={!setupDone} className="btn-ghost">
                  チームを作成
                </button>
              </div>
              {justCreatedId && (
                <div className="muted" style={{ marginTop: 8 }}>
                  作成したチームID：
                  <code style={{ fontWeight: 700 }}>{justCreatedId}</code>{" "}
            ...
            <div className="subcard" style={{ border: "1px solid #eee", borderRadius: 10, padding: 12, marginBottom: 10 }}>
              ...
              <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
                <input placeholder="チームIDを入力" value={groupId} onChange={(e) => setGroupId(e.target.value)} style={{ padding: 6, minWidth: 240 }} disabled={!setupDone} />
                <button onClick={joinGroup} disabled={!setupDone || !groupId.trim()} className="btn-ghost">
                  参加
                </button>
              ...
              <div className="muted" style={{ marginTop: 6 }}>※ 「参加」で自分をチームに登録、「表示/更新」でチーム全体の最新状況を取得します。</div>
            </div>
            ...
            {/* 一覧 */}
            <div style={{ marginTop: 16 }}>
              {groupMembers.length === 0 && <div className="muted">まだメンバー情報がありません。上の「参加」または「送信」をお試しください。</div>}
        ...
        {isUnityOpen && (
          <div
            ...
            style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }}
            onClick={(e) => {
              if (e.target === e.currentTarget) setIsUnityOpen(false);
            }}
          >
            <div
              style={{ width: "min(1280px, 96vw)", background: "#fff", borderRadius: 12, boxShadow: "0 10px 30px rgba(0,0,0,.2)", padding: 12 }}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="row" style={{ justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <h3 style={{ margin: 0 }}>3Dシミュレーション</h3>
                <button onClick={() => setIsUnityOpen(false)} className="btn-ghost">
                  閉じる
                </button>
              </div>
              <p className="muted" style={{ marginTop: 0 }}>岐阜市が想定している岐阜駅周辺の浸水状況を確認してみよう！！</p>
              <UnityWebGLPlayer />
            </div>
          </div>
        )}
